...
function Crops({ cart, setCart }) {
  ...
  useEffect(function () {
    async function getCrops() {
      try {
        ...
        const cropsData = data.data.data;
  ...
  useEffect(() => {
    ...
    // Extract crop, user, and price from the URL
    ...
    const storeData = async () => {
      try {
        const response = await fetch(
          `http://localhost:5173/api/v1/bookings/booking`,
          {
            method: "GET",
            headers: {
            ...
            credentials: "include",
            params: JSON.stringify({ crop, user, price }),
          }
        );
        const data = await response.json();
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/crops/search?name=${search}`);
      if (!response.ok)
        throw new Error("Something went wrong with fetching crops");

      const data = await response.json();
      console.log("Search response data:", data);
      let cropsData = data.data.crop;
